...
galRenderer.effect.bgSwitch.mask = function(fn) {
  ...
  bgBitmapNew.image = boatgal.queue.getResult(boatgal.status.renderObjDiff.bg.id);
  ...
  function finish() {
    bgBitmapOld.image = boatgal.queue.getResult(boatgal.status.renderObjDiff.bg.id);
