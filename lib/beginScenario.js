 ...
 */
 boatgal.beginScenario = function() {
  ...
  if (boatgal.isLoadSave) {
    ...
    /**
     * 读档
     * 复制config内的status,以免游戏过程中改变config内的status,
     * 这样，游戏过程中也可以读取boatgal.config对象内的档
     */
    boatgal.status = JSON.parse(JSON.stringify(boatgal.config.status));
  } else {

    /**
     * 重新开始
     * 则把boatgal.scenario.part0[0],赋值给boatgal.status.stage
     */
    boatgal.status.stage = boatgal.scenario.part0[0];
  ...
  //读入下一个change,把change写入boatgal.status.stage
  function readNextChange() {
    ...
    //遍历change中的属性,赋值到boatgal.status.stage
    for (var pro in change) {
      boatgal.status.stage[pro] = change[pro];
    }
  }
 };